...
import {
  fromFullNameToDirectoryItemName,
  fullNameToFileDescriptor,
  seperateDirectoryFromFile,
} from "./util/dir";
...
export async function traverseMainDirectory(): Promise<DirectoryItem> {
  ...
  while (activeDirectoryItem) {
    ...
    if (cursor.index === 0) {
      ...
      try {
        activeDirectoryItem.rawDirectoryNames = (await readdir(
          activeDirectoryItem.absPath
        )) as string[];
      } catch (err) {
        criticalErrorHandler(err);
      }

      const { files: fileNames, directories: subdirectoryNames } =
        await seperateDirectoryFromFile(
          activeDirectoryItem.rawDirectoryNames,
          activeDirectoryItem.absPath
        );
      ...
      for (const fileName of activeDirectoryItem.fileNames) {
        const exported = await import(
          pathToFileURL(resolve(absPath, fileName)).toString()
        );
    ...
    if (
      ...
      activeDirectoryItem.subdirectories.length
    ) {
